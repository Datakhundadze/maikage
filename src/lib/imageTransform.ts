...
//         ?width=<w>&height=<w>&resize=contain&quality=<q>
//
// height + resize=contain are REQUIRED, not optional: width-only requests
// default to resize=cover, which CROPS the image server-side (broke catalog
// previews — designs lost their sides; reverted in 06392b9). A square
// contain box was verified on the live project to downscale proportionally
// with no cropping and no padding (2923×3601 → 487×600, identical ratio),
// which keeps the composite's aspect-ratio math (printAR) correct.
//
...
export function transformedDisplayUrl(url: string, { width, quality = 65 }: DisplayTransform): string {
  ...
  return `${rewritten}${sep}width=${width}&height=${width}&resize=contain&quality=${quality}`;
}
