...
import { Upload, Sparkles, Sticker, Mail, Phone, ArrowRight, Shield, Zap, Users, BadgeDollarSign } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white overflow-hidden">
      ...
      {/* Top bar: theme toggle + contacts */}
      <div className="relative z-20 flex items-center justify-between px-4 sm:px-6 py-3">
        <button
          onClick={toggleTheme}
          className="rounded-full p-2 text-gray-400 dark:text-white/40 hover:text-gray-800 dark:hover:text-white/80 transition-colors"
        >
          {theme === "dark" ? "☀️" : "🌙"}
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="mailto:maika@maika.ge" className="text-xs text-gray-500 dark:text-white/50 hover:text-gray-800 dark:hover:text-white/80 transition-colors hidden sm:inline-flex items-center gap-1">
            <Mail className="h-3 w-3" /> maika@maika.ge
          </a>
          <span className="hidden sm:inline text-gray-300 dark:text-white/20">|</span>
          {/* Social icons */}
          <div className="flex items-center gap-2">
            <a href="https://wa.me/995599050807" target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-white/40 hover:text-green-500 transition-colors" aria-label="WhatsApp">
            ...
            <a href="https://facebook.com/maika.ge" target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-white/40 hover:text-blue-500 transition-colors" aria-label="Facebook">
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://www.tiktok.com/@maika.ge" target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-white/40 hover:text-gray-800 dark:hover:text-white transition-colors" aria-label="TikTok">
            ...
            <a href="https://www.instagram.com/maika.ge_/" target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-white/40 hover:text-pink-500 transition-colors" aria-label="Instagram">
      ...
      <div className="relative z-10 flex flex-col items-center min-h-[calc(100vh-52px)] px-4 py-8 sm:py-16">
        ...
        {/* Mode Cards */}
        <div className="grid w-full max-w-4xl gap-6 sm:grid-cols-3 mb-16 sm:mb-20">
          {/* Simple Mode */}
          <button
        ...
        <div className="mt-auto flex flex-col items-center gap-3 text-sm">
          <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
            <button onClick={() => setMode("terms")} className="text-gray-500 dark:text-white/50 hover:text-gray-800 dark:hover:text-white/80 transition-colors text-xs">
              წესები და პირობები
            </button>
            <span className="text-gray-300 dark:text-white/30">•</span>
            <button onClick={() => setMode("privacy")} className="text-gray-500 dark:text-white/50 hover:text-gray-800 dark:hover:text-white/80 transition-colors text-xs">
              კონფიდენციალურობა
            </button>
          </div>
          <p className="text-xs text-gray-400 dark:text-white/40">© 2026 maika.ge</p>
        </div>
      </div>
    </div>
  );
