// ⚠️ COPY of supabase/functions/gemini-proxy/redactPii.ts, byte-for-byte below
// this header. Supabase edge functions are deployed one directory at a time
// and cannot import across function folders, and gemini-proxy is not to be
// touched to share it. If the original changes, re-copy it here. The vitest
// suite (src/test/metaWebhook.test.ts) asserts the two files are identical
// below this header so drift is caught in CI, not in production.
//
// The masking is the same one chat_logs gets: a customer who types a phone
// number or an email into a DM has it stored as [phone:***NN] / [email:x***@d].
// Our own published contact details are left intact for the same reason as
// in the original.
